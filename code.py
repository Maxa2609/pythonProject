...
bot = telebot.TeleBot('5401931136:AAGvrnvVEUjhKsnm6yoCVpAlGlZTZQHem8g')
...
def call_data(index1, index2, call):
    ...
    if hod % 2 == 0 and call.message.chat.id == user2.id:
        if board[index1][index2] == ':white_medium_square:':
...
def check_win(board):
    for i in range(3):
        if board[i][1] == board[i][0] and board[i][1] == board[i][2] and board[i][1] != ':white_medium_square:':
            return True, board[i][0]
    for i in range(3):
        if board[1][i] == board[0][i] and board[1][i] == board[2][i] and board[1][i] != ':white_medium_square:':
            return True, board[1][i]
    for i in range(2):
        if board[0][0] == board[1][1] and board[1][1] != ':white_medium_square:' and board[1][1] == board[2][2]:
            return True, board[0][0]
        elif board[1][1] != ':white_medium_square:' and board[1][1] == board[0][2] and board[0][2] == board[2][0]:
...
def chat(message):
    ...
    board = [[':white_medium_square:'] * 3 for i in range(3)]
    players = [user1, user2]
    hod = 0
    keyboard = types.InlineKeyboardMarkup(row_width=3)
    b1 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=1)
    b2 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=2)
    b3 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=3)
    b4 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=4)
    b5 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=5)
    b6 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=6)
    b7 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=7)
    b8 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=8)
    b9 = types.InlineKeyboardButton(text=':white_medium_square:', callback_data=9)
    keyboard.add(b1, b2, b3, b4, b5, b6, b7, b8, b9)
    if bot_send:
        make_bot_false()
